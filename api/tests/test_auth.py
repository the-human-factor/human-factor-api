...
def test_user_registration(client):
  ...
  resp = client.post(
    url_for("userregister"),
    json=dict(
      fullName="Cool Person", email="coolperson@example.com", password="hunter2"
    ),
  )
...
def test_user_registration_missing_keys(client):
  ...
  resp = client.post(
    url_for("userregister"),
    json=dict(email="coolperson@example.com", password="hunter2"),
  )
...
def test_user_login(client, user):
  resp = client.post(
    url_for("userlogin"), json=dict(email=user.email, password="hunter2")
  )
...
def test_user_login_missing_key(client, user):
  resp = client.post(url_for("userlogin"), json=dict(email=user.email))

  assert resp.status_code == 400

  resp = client.post(url_for("userlogin"), json=dict(password="hunter2"))
...
def test_wrong_password_login(client, user):
  resp = client.post(
    url_for("userlogin"), json=dict(email=user.email, password="invalid-password")
  )
...
def test_missing_fields_login(client, user):
  resp = client.post(url_for("userlogin"), json={})
...
def test_refresh_token(client, refresh_token):
  resp = client.post(
    url_for("userrefresh"), headers={"Authorization": f"Bearer {refresh_token}"}
  )
...
def test_change_password_success(client, access_token):
  ...
  resp = client.put(
    url_for("userpassword"),
    json=dict(oldPassword="hunter2", password=new_password),
...
def test_change_password_failure(client, access_token):
  resp = client.put(
    url_for("userpassword"),
    json=dict(oldPassword="hunter3", password="Test123123"),  # wrong pass
    headers={"Authorization": f"Bearer {access_token}"},
  )
